...
export function createScanner(deps) {
  ...
  async function scanRoot(root, home) {
    ...
    try {
      ...
      // The real adapter resolves a path WITHOUT requiring it to exist — it
      // returns a target for a directory that was never created — so a failing
      // listDir covers both "never created" and a genuine read failure.
      // stat() is what separates them: its contract is
      // `(target) => Promise<FsInfo | undefined>`, and undefined means absent.
      // Reporting a missing root as `unreadable` claims an I/O problem that
      // never happened, which is how the panel showed `.agents/skills` in a
      // checkout that has no such directory.
      let missing = false;
      if (typeof fs.stat === 'function') {
        try {
          missing = (await fs.stat(dir)) === undefined;
        } catch {
          missing = false;
        }
      }
      return { root, status: missing ? 'absent' : 'unreadable', rows: [] };
    }
