...
/** Default scanned roots. Order matters: the first root to supply a name wins. */
export function defaultRoots(home, project) {
  return [
    project + '/.dsh/skills',
...
export function createScanner(deps) {
  ...
  // Defaults come from the host process so the bundle works without config.
  // `process` is present in the Host (Node) context; the guards keep the module
  // importable anywhere.
  const envHome = (typeof process !== 'undefined' && process.env) ? process.env.HOME : undefined;
  const cwd = (typeof process !== 'undefined' && process.cwd) ? process.cwd() : undefined;

  const home = cfg.home ?? envHome ?? '/Users/taozi';
  const project = cfg.project ?? cwd ?? home;
  const roots = cfg.roots ?? defaultRoots(home, project);
  ...
  async function poll() {
    ...
    for (const root of roots) {
      ...
      perRoot.push({
        ...
        short: shortPath(root, home, project),
        status: r.status,
        found: r.rows.length,
        counted,
      });
