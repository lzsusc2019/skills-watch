window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function apply(ctx) {
			...
			// The mounted namespace is a Cordis Service of its own, registered by
			// api-gateway under the key `remote.<namespace>` — it is NOT a property
			// of the `remote` service. `ctx.remote.skillsWatch` is therefore
			// undefined forever; the namespace has to be read through ctx.reflect,
			// exactly as dsh-at-file reads `remote.atFile` after $mount resolves.
			function namespace() {
				const reflect = ctx.reflect;
				if (reflect === undefined || typeof reflect.get !== "function") return undefined;
				const mounted = reflect.get("remote.skillsWatch");
				return mounted === undefined ? undefined : mounted;
			}

			// Every mounted Remote method resolves with a Result envelope, never
			// with the bare business value — api-gateway's install() returns
			// `{ ok: true, value: parse(descriptor.result, …) }` on success and
			// `{ ok: false, error }` otherwise. A failure has to become a thrown
			// error here: handed on untouched, the renderer sees an envelope with
			// no `skills`, `roots` or `summary` fields and draws an empty table with
			...
			function unwrapRemote(raw) {
				...
				if (raw.ok === false) {
					...
					const message = err && typeof err.message === "string" ? err.message : "remote call failed";
					throw new Error(code + message);
				}
				return raw.ok === true ? raw.value : raw;
			}
			...
			async function refresh(cwdOverride) {
				...
				try {
					...
					const raw = await ns.list(cwd ? { cwd: cwd } : {});
					// Every mounted Remote method answers with a Result envelope, never
					// with the bare business value (api-gateway client install():
					// `{ ok: true, value: parse(descriptor.result, …) }` on success,
					// `{ ok: false, error }` otherwise). Reading the snapshot directly
					// off the envelope yields an empty table with no error to show.
					const res = unwrapRemote(raw);
					publish(Object.assign({}, current(), {
			...
			//
			// A $mount rejection used to escape this effect into Cordis' logger
			// only, leaving the store at its initial state: the badge sat on
			// "skills..." and the panel on "not checked yet" with no hint that
			// anything had gone wrong. Surface it in the store instead, so a
			// failed mount is always visible in the panel.
			ctx.effect(async () => {
				let dispose;
				try {
					...
					publish(Object.assign({}, current(), {
						error: "mount failed: " + ((err && err.message) ? err.message : String(err)),
						loading: false,
					}));
					return undefined;
				}
