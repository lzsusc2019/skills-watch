...
class SkillsWatchGateway extends TypertRemoteService {
   ...
   * @param {object} [config] - `roots` replaces the inferred list; `extraRoots`
   *   appends to it; `home` / `project` / `dshHome` / `agentsHome` pin the bases
   *   the defaults are derived from. See lib/scan.js `resolveRoots`.
   */
   ...
   *
   * @param {{cwd?: string}} [request] - the calling session's working directory.
   *   Project roots are discovered from it the same way the Harness discovers
   *   them, so a skill installed into a repository's `.dsh/skills` is found even
   *   when the Host process was launched elsewhere. Absent, the Host process cwd
   *   is used.
   * @returns {Promise<object>} lossless-JSON snapshot: skills, roots, projectRoot, summary, note, polledAt.
   */
  async list(request) {
    const req = (request && typeof request === 'object') ? request : {};
    const r = await this.scanner.poll(req);
    return {
      skills: r.skills,
      roots: r.roots,
