// skills-watch — Client half
//
// Runs in the browser. Registers two slot occupants that share one store:
//
//   conversation.session.header.utilities  → the status badge
//   shell.overlay                          → the floating panel
//
// Three facts this code depends on, each of which cost real debugging time:
//
//  1. Mutating a plain object does NOT re-render. Both occupants read a shared
//     store through React.useState + React.useEffect; `publish()` notifies
//     subscribers and that is what repaints them.
//  2. `shell.overlay` is click-through by design — "the layer itself is
//     click-through; entries opt back into pointer events". Without
//     `pointerEvents: 'auto'` the panel renders but every control is dead.
//  3. Slot render functions are React components, so hooks work in them.
//
// Load it by passing this file's body as `code.client` to cordis_define.
...
return {
  apply(ctx) {
    ...
    ctx.effect(function () { return styles.insert(css); }, 'skills-watch-styles');

    // ─── Shared store ─────────────────────────────────────────────────────
    const listeners = new Set();
    let snapshot = {
      open: false, showIdle: false, rows: [], roots: [],
      summary: { total: 0, managed: 0, unmanaged: 0, upToDate: 0, behind: 0, unknown: 0 },
      note: null, polledAt: null, error: null, loading: true,
    };
    ...
    async function refresh() {
      publish(Object.assign({}, current(), { loading: true }));
      try {
        const res = await host.call('skills-poll', {});
        publish(Object.assign({}, current(), {
          rows: (res && Array.isArray(res.skills)) ? res.skills : [],
          roots: (res && Array.isArray(res.roots)) ? res.roots : [],
    ...
    // The one hook both occupants use. Subscribing after the first render and
    // re-syncing immediately covers a publish that landed in between.
    ...
    // Why a root contributed nothing.
    ...
    slots.inject('conversation.session.header.utilities', function () {
      return slots.register(
        { name: 'conversation.session.header.utilities', id: 'skills-watch-badge', order: 30, label: 'skills-watch' },
        function SkillsWatchBadge() {
          const s = useSnapshot();
          const behind = (s.summary && s.summary.behind) || 0;
          ...
          return React.createElement(
            ...
            {
              onClick: toggle,
              title: s.error ? ('poll failed: ' + s.error)
            ...
            badgeText(s),
            React.createElement('span', { style: { fontSize: 10, opacity: 0.75 } }, s.open ? '\u25B2' : '\u25BC'),
          );
        },
      );
    });
    ...
    const STATE_LABEL = {
      behind: 'behind upstream', up_to_date: 'up to date',
      unknown: 'check failed', unmanaged: 'unmanaged',
    };

    // ─── Floating panel ───────────────────────────────────────────────────
    slots.inject('shell.overlay', function () {
      return slots.register(
        { name: 'shell.overlay', id: 'skills-watch-overlay', order: 50, label: 'skills-watch panel' },
        function SkillsWatchPanel() {
          ...
          const head = React.createElement(
            ...
            React.createElement('button', {
              className: 'sw-close', title: 'Close',
              onClick: function () { publish(Object.assign({}, current(), { open: false })); },
            }, '\u00D7'),
          );
          ...
          // Source line: contributing roots stay visible, the rest collapse
          // into one `+N others` button that expands in place.
          const active = roots.filter(function (r) { return (r.counted || 0) > 0; });
          const idle = roots.filter(function (r) { return !(r.counted || 0); });
          const srcChips = [React.createElement('span', { key: 'lbl', className: 'sw-src-label' }, 'from')];
          ...
          if (active.length === 0 && idle.length === 0) {
            ...
            active.forEach(function (r, idx) {
              ...
              srcChips.push(React.createElement(
                'span',
                { key: 'a' + idx, className: 'sw-root', title: r.root + '  \u2014  ' + (r.counted || 0) + ' skills' },
            ...
            if (idle.length > 0) {
              const idleTip = idle.map(function (r) {
                return r.root + '  \u2014  ' + idleReason(r);
              }).join('\n');
              srcChips.push(React.createElement(
                'button',
                {
            ...
            if (s.showIdle && idle.length > 0) {
              idle.forEach(function (r, idx) {
                srcChips.push(React.createElement(
                  'span',
                  { key: 'i' + idx, className: 'sw-root is-idle', title: r.root + '  \u2014  ' + idleReason(r) },
                  React.createElement('span', { className: 'sw-rootcode' }, r.short || r.display || r.root),
          ...
          const list = rows.length === 0
            ...
            : rows.map(function (r) {
                ...
                const kids = [
                  ...
                  React.createElement('div', { key: 'st', className: 'sw-state', title: r.detail || '' }, STATE_LABEL[r.status] || r.status),
                ];
                // Compressed one-liner; the full description stays on hover.
                const line = r.summary || r.description;
                if (line) {
                  kids.push(React.createElement('div', {
                    key: 'sum', className: 'sw-sum', title: r.description || line,
                  }, line));
                }
                if (r.status === 'behind') {
          ...
          return React.createElement(
            'div',
            { className: 'sw-panel', 'data-skills-watch': 'panel' },
            head,
    ...
    // First poll, once the Client half is mounted.
    refresh();
  },
};
